"""Fast Discrete Dispersion Measure Transform (FDMT) kernel.

Extracted from fdmt_search.py so the transform can be imported and unit-tested
in isolation (the search script has top-level code that runs on import, and the
original FDMT_initialization referenced a module-global `ds_max`).

This is a faithful copy of the kernel as it runs in production. The ONLY change
is in FDMT_initialization: it now uses its `maxDT` argument instead of reaching
out to a global named `ds_max`. That global always equalled this argument at the
call site, so the change is behaviour-preserving -- it just makes the function
self-contained.

Original code from https://github.com/iansbrown/FRB-FDMT-Search, mostly written
by Bruce Wu with modifications by Ian Brown. See also the fdmt.m codes under
https://webhome.weizmann.ac.il/home/eofek/matlab/FunList.html
Reference: Zackay & Ofek (2014/2017), "An accurate and efficient algorithm for
detection of radio bursts with an unknown dispersion measure".
"""
import logging
...
def FDMT_initialization(Image, f_min, f_max, maxDT, dtype):
   ...
   Input:
     ...
     maxDT - the maximal delay (in sample bins) of the maximal dispersion.
   ...
   deltaT = int(np.ceil((maxDT - 1) * (f_min**-2 - (f_min + deltaF)**-2) / (f_min**-2 - f_max**-2)))
...
def FDMT(Image, f_min, f_max, maxDT, dtype):
    """
    ...
    Input:
      ...
      maxDT - the maximal delay (in time/sample bins) of the maximal dispersion
        Appears in the paper as N_{\\Delta}
        A typical input is maxDT = N_f
    ...
    Output:
      The dispersion measure transform of the Input matrix.
        The output dimensions are [maxDT + 1, Input.shape[1]]
        i.e. row index == total delay across the band (in samples), 0..maxDT.
    For details, see algorithm 1 in Zackay & Ofek (2014)
    """
    N_f, N_s = Image.shape
    f = int(np.log2(N_f))
    ...
    # Initialize
    State = FDMT_initialization(Image, f_min, f_max, maxDT, dtype)
    logger.debug('FDMT initialized')
    # Iterate
    for i_t in range(1, f + 1):
      State = FDMT_iteration(State, maxDT, N_f, f_min, f_max, i_t, dtype)
